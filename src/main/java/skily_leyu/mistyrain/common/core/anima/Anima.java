...
public class Anima {
    ...
    public int compareAmount(Anima anima) {
        int thisAmount = (int) (Math.pow(MRConfig.PotRule.ANIMA_COMBINE_AMOUNT.get(), this.getLevel().ordinal())
                * this.getAmount());
        int thatAmount = (int) (Math.pow(MRConfig.PotRule.ANIMA_COMBINE_AMOUNT.get(), anima.getLevel().ordinal())
                * anima.getAmount());
    ...
    public void addAnima(Anima anima) {
        if (anima != null && this.isAnima(anima.getType())) {
            ...
            if (levelInx >= 0) {
                this.amount += (int) (Math.pow(MRConfig.PotRule.ANIMA_COMBINE_AMOUNT.get(), levelInx)
                        * anima.getAmount());
            }
        }
    }
